...
import zk.bulletproofs.VariableType;
...
public class SharpProver {
	...
	public SharpProof generateRangeProof(BigInteger[] x, BigInteger r_x, BigInteger B, int R, BigInteger L) {
		...
		Commitment C_x = prover.commit(Utils.scalar(sum_x), Utils.scalar(r_x));
		...
		Commitment C_y = prover.commit(Utils.scalar(sum_y_mu), Utils.scalar(r_y));
		...
		Commitment D_x = prover.commit(Utils.scalar(sum_tx), Utils.scalar(tilde_rx));
		...
		Commitment D_y = prover.commit(Utils.scalar(sum_ty_tmu), Utils.scalar(tilde_ry));
		...
		ECPoint ec_sum_alpha_1_pos = pc.commit(Utils.scalar(sum_alpha_1_pos), Utils.scalar(r_star.multiply(BigInteger.TWO))).decompress();
		ECPoint ec_sum_alpha_1_neg = pc.commit(Utils.scalar(sum_alpha_1_neg), Utils.scalar(r_star)).decompress();
		ECPoint ec_sum_alpha_1 = ec_sum_alpha_1_pos.subtract(ec_sum_alpha_1_neg).compress();
		
		Commitment C_star = new Commitment(ec_sum_alpha_1, new Variable(VariableType.committed, BigInteger.ZERO));
		...
		Commitment D_star = prover.commit(Utils.scalar(sum_alpha_0), Utils.scalar(tilde_r_star));
...
//		System.out.println(gamma_star);
		...
		return new SharpProof(C_x, C_y, D_x, D_y, C_star, D_star, t_x, t_y, t_star, zeta, z_x, z_y, tau, d);
	}
}
